...
- Если validation marker утрачен, exact version-history recovery может восстановить только тот же mapping и только при точном совпадении текущих source, обоих prompts и используемого stage contract с historical версией. Для исторической модели `semanticModelRevision=8` / tree `3` прежний valid marker после ротации signing secret является лишь migration attestation: обычный `Сохранить` заново парсит source, проверяет все contracts, компилирует текущую модель и подписывает её текущим ключом. Такая attestation никогда не является runtime-valid mapping и не создаёт legacy compatibility.
