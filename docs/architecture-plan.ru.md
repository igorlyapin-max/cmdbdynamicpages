...
## Domain-aware пути и ссылки в runtime

Кэш каталога хранит не только имена классов/атрибутов, но и metadata путей: домен, описание домена, кардинальность, направление, исходный и целевой класс. Designer использует эти признаки в `Группе объектов`, чтобы фильтровать выпадающие списки атрибутов/путей по типу связи. Это не меняет права пользователя и не расширяет модель CMDBuild: фильтр только помогает выбрать правильный path из уже видимого каталога.

Runtime результат хранит для каждой ячейки итоговой таблицы lightweight `cellMeta`: источник (`ВыборкаX`), класс/идентификатор карточки, имя атрибута и путь домена, если он известен. Дополнительно backend формирует готовые внутренние URL на карточки, участвовавшие в строке результата: `${mysource.sourceURLВыборка1}`, `${mysource.sourceURLВыборка2}`, `${mysource.sourceURLSelection1}` и аналогичные переменные по обнаруженным префиксам выборок. Визуализация может использовать эти данные для построения ссылок через шаблоны `${mysource.value}`, `${mysource.source}`, `${mysource.sourceClass}`, `${mysource.sourceId}`, `${mysource.attribute}`, `${mysource.domainPath}`, `${row.<column>}` и `${param.<name>}`.

Backend и client блокируют небезопасные URL-схемы `javascript:`, `data:` и `vbscript:`. Если шаблон ссылки дает небезопасный или пустой URL, ячейка отображается как обычный текст.
